...
def create_grid(c, col, row, data=None, color=True): #https://www.coder.work/article/4933449
    zeor_color = '#ffe4c4' if color else "lightslategray"
    one_color = '#9D9D9D'  if color else "gold"
    #zeor_color = '#778899' if color else "lightslategray"
    #one_color = '#ffd700'  if color else "gold"
...
def focus(event):
    for i in range(109):
        ...
        elif(event.widget.itemcget(i ,'fill')=='#ffe4c4'):
            event.widget.itemconfig(i, fill='bisque')
def unfocus(event):
    for i in range(109):
        if(event.widget.itemcget(i ,'fill')=='black'):  #取得舊有屬性
            event.widget.itemconfig(i, fill='#9D9D9D')
        elif(event.widget.itemcget(i ,'fill')=='bisque'):
            event.widget.itemconfig(i, fill='#ffe4c4')    
